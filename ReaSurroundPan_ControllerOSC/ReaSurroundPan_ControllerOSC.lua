...
function updatePanners()
  ...
  -- cicla tutte le tracce selezionate
                ...
                if ch_prefix and suffix then
                    if suffix == "X" then
                        if osc_touch == true then
                            ...
                            local msg1 = osc.encode('ReaSurroundPan_X', paramVal)-- encoding osc message with the address `/track 1/fx parameters` the values are a random integer, a flot and a string
                        ...
                        elseif suffix == "Y" then
                            if osc_touch == true then
                                ...
                                local paramVal = reaper.TrackFX_GetParam(track, fx, p)
                                local msg1 = osc.encode('ReaSurroundPan_Y', paramVal)-- encoding osc message with the address `/track 1/fx parameters` the values are a random integer, a flot and a string
                                controller_udp:sendto(msg1, REAPER_IP, DEVICE_PORT)
                            end
                        end
                    end
