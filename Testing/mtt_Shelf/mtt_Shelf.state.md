...
updated: 2026-09-22
- Done:
  - The drag preview window that follows the cursor now renders the actual favorite button (same FX style: dark-green border, transparent fill, name at normal size) instead of the "Drag: <name>" text.
- Not done:
  - None.
- Waiting on:
  - In-REAPER run: drag an FX favorite and check the preview window shows the button (no "Drag:" text), then drop on a track.
