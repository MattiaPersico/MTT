...
-- Guard nel caso improbabile di mancato highest_track
if not highest_track then
    reaper.PreventUIRefresh(-1)
    return
end
...
compute_group_peaks(ref_groups) -- per coerenza, anche se usiamo start_pos come riferimento
...
-- Allineamento: peak target -> start ref
for track, items in pairs(track_items) do
    table.sort(items, function(a,b) return a.pos < b.pos end)
    ...
    for idx, ref_group in ipairs(ref_groups) do
        ...
        local offset = ref_group.start_pos - target_group.peak_time
    ...
    for i, g in ipairs(groups) do
        if prev_end then
            ...
            if INCLUDE_TOUCHING then
                overlap = g.start_pos < prev_end - EPS
            else
                overlap = g.start_pos < prev_end - EPS
            end
            if overlap then
...
reaper.Undo_EndBlock("Allinea picchi (target) agli start (ref) con grouping crossfade", -1)
reaper.PreventUIRefresh(-1)
