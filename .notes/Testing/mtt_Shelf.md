...
updated: 2026-09-17 · commit da4e8d1 · tree: modified: Testing/mtt_Shelf.lua, staged: .notes/
...
- Waiting on: a run in REAPER to confirm the one-row layout (the 100px input width is a guess and may need adjusting), that toggling a format hides and restores both its normal and instrument rows, that the five-line filter file persists and is re-applied on reload, that CLAP/VSTi/CLAPi/VST3i rows filter, plus the still-unconfirmed items from before (focus, capped list, width limit) — and a commit: the work exists only in the working tree.
